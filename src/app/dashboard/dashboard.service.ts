...
export class DashboardService {
  ...
  getDashboardData(): Observable<any> {
     ...
     const period = this.reportPeriodService.getPeriodByMonth(today.getMonth(), today.getFullYear());
     const todayIso = today.toISOString();

     return forkJoin({
       system: forkJoin({
         users: this.api.get<any[]>('/users').pipe(catchError(() => of([]))),
         companies: this.api.get<any[]>('/companies').pipe(catchError(() => of([])))
       }),
       employees: this.employeesService.getAll().pipe(catchError(() => of([]))),
       billing: this.mealsService.getWeeklySummary(period.startIso, period.endIso).pipe(catchError(() => of({ totalQty: 0, totalValue: 0 }))),
       todayMeals: this.mealsService.getDailyMeals(todayIso).pipe(map(m => m.length), catchError(() => of(0))),
       integration: this.api.get<any>('/integrations/dominio/config').pipe(catchError(() => of(null))),
