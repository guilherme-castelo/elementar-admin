...
export class RoleFormComponent implements OnInit {
  ...
  isGroupPartiallySelected(groupName: string): boolean {
    ...
    if (!group) return false;
    const currentperms = this.form.get('permissions')?.value as string[];
    const selectedCount = group.permissions.filter(p => currentperms.includes(p.slug)).length;
  ...
  save() {
    ...
    request$.subscribe({
      next: () => {
        this.snackBar.open('Cargo salvo com sucesso!', 'Fechar', { duration: 3000 });
        this.router.navigate(['/roles']);
      },
      error: (err) => {
        console.error(err);
        this.snackBar.open('Erro ao salvar cargo.', 'Fechar', { duration: 3000 });
      }
    });
  }
}
