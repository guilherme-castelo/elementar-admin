...
@Component({
  ...
  template: `
    ...
    <mat-dialog-content class="mat-typography min-w-[500px]">
      ...
      <!-- STEP 1: Upload -->
      <div *ngIf="!analysisResult" class="flex flex-col items-center justify-center p-8 border-2 border-dashed border-neutral-300 rounded-lg hover:bg-neutral-50 transition-colors cursor-pointer" onclick="document.getElementById('fileInput').click()">
      ...
      <div *ngIf="analysisResult && !isImporting && !importComplete">
         <div class="flex gap-4 mb-4">
            <div class="flex-1 bg-green-50 p-4 rounded border border-green-100">
               ...
               <div class="text-2xl text-green-600">{{ analysisResult.valid.length }}</div>
            </div>
            <div class="flex-1 bg-red-50 p-4 rounded border border-red-100">
               <div class="text-red-800 text-sm font-bold">Inválidos</div>
               <div class="text-2xl text-red-600">{{ analysisResult.invalid.length }}</div>
            </div>
         </div>
            ...
            <table class="w-full text-xs text-left">
               ...
               <tbody>
                 <tr *ngFor="let err of analysisResult.invalid" class="border-t">
                   <td class="p-2 truncate max-w-[150px]">{{ err.row.firstName || err.row.matricula || 'Linha desconhecida' }}</td>
         ...
         <div class="text-sm text-neutral-500" *ngIf="analysisResult.valid.length > 0">
            Pronto para importar {{ analysisResult.valid.length }} funcionários.
         </div>
         <div class="text-sm text-red-500 font-medium" *ngIf="analysisResult.valid.length === 0">
      ...
      <div *ngIf="isImporting" class="py-6">
         <p class="text-center text-sm text-neutral-500 mb-2">Importando {{ analysisResult?.valid?.length }} registros...</p>
         <mat-progress-bar mode="indeterminate"></mat-progress-bar>
      </div>
    ...
    <mat-dialog-actions align="end">
      ...
      <button mat-raised-button color="primary" 
              [disabled]="!analysisResult || analysisResult.valid.length === 0 || isImporting || importComplete"
              (click)="confirmImport()"
              *ngIf="!importComplete">
         Importar
      </button>
...
export class EmployeeImportDialogComponent {
  ...
  onFileSelected(event: any) {
    ...
    this.importService.parseFile(file).then(data => {
      // Validate Batch
  ...
  confirmImport() {
    ...
    this.importService.importEmployees(this.analysisResult.valid).subscribe({
      ...
      error: (err) => {
        alert('Erro crítico na importação: ' + err);
        this.isImporting = false;
      }
    });
  }
