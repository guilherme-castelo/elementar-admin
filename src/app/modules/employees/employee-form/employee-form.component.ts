...
import {
  ...
  DismissalAction,
} from '../../employees/dismissal-meals-dialog/dismissal-meals-dialog.component';
  ...
  template: `
    <div class="p-6 max-w-4xl mx-auto">
      ...
      <form
        ...
        (ngSubmit)="onSubmit()"
        class="p-8 rounded-lg shadow-sm border border-neutral-200"
      >
        <!-- Identificação -->
        <h2 class="text-lg font-semibold mb-4 text-primary-700 border-b pb-2">
          Identificação
        </h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <mat-form-field appearance="outline">
            ...
            />
            <mat-error
              *ngIf="employeeForm.get('matricula')?.hasError('required')"
          ...
          <mat-form-field appearance="outline">
            ...
            />
            <mat-error *ngIf="employeeForm.get('cpf')?.hasError('required')"
              >Obrigatório</mat-error
            >
          </mat-form-field>
        </div>
        ...
        <!-- Cargo e Setor -->
        <h2
          class="text-lg font-semibold mb-4 mt-6 text-primary-700 border-b pb-2"
        >
          Cargo e Setor
        </h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <mat-form-field appearance="outline">
            ...
            />
            <mat-error *ngIf="employeeForm.get('funcao')?.hasError('required')"
              >Obrigatório</mat-error
            >
          </mat-form-field>
          ...
          <mat-form-field appearance="outline">
            ...
            <mat-error *ngIf="employeeForm.get('setor')?.hasError('required')"
              >Obrigatório</mat-error
            >
          </mat-form-field>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <mat-form-field appearance="outline">
            ...
            <mat-error
              *ngIf="employeeForm.get('dataAdmissao')?.hasError('required')"
              >Obrigatório</mat-error
            >
          </mat-form-field>
...
export class EmployeeFormComponent implements OnInit {
    ...
    this.employeesService.getById(id).subscribe((employee) => {
      this.employeeForm.patchValue({
        ...
        dataAdmissao: employee.dataAdmissao
          ? new Date(employee.dataAdmissao)
          : null,
        dataDemissao: employee.dataDemissao
          ? new Date(employee.dataDemissao)
          : null,
      });
    });
  }
  ...
  onSubmit() {
    ...
    // Check for dismissal and linked meals
    const hasDismissalDate = !!formValue.dataDemissao;
    const checkDismissal$ =
      this.isEdit && hasDismissalDate
        ? this.employeesService.countLinkedMeals(this.employeeId!).pipe(
            switchMap((res) => {
              if (res.count > 0) {
    ...
    checkDismissal$.subscribe(
      (mealsAction: DismissalAction | null | undefined) => {
        // If result is undefined, it means dialog was opened but somehow closed without value (e.g. if we allowed backdrop close, but we didn't).
        // Actually with disableClose:true, user must click a button.
        // But if we handle 'close' in dialog to return undefined, we stop.
        if (mealsAction === undefined) {
        ...
        const employeeData = {
          ...
          dataAdmissao: formValue.dataAdmissao
            ? formValue.dataAdmissao.toISOString()
            : null,
          dataDemissao: formValue.dataDemissao
            ? formValue.dataDemissao.toISOString()
            : null,
          mealsAction: mealsAction, // Pass the chosen action
        };
